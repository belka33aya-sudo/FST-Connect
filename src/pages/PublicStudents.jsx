...
const PublicStudents = () => {
  ...
  // Generate the official lists (documents) based on filieres and their duration (years)
  const generateListDocuments = () => {
    let docs = [];
    let idCounter = 1;

    filieres.forEach(filiere => {
      // For each year of the filiere's duration
      for (let year = 1; year <= filiere.duree; year++) {
        let yearLabel = '';
        if (filiere.level === "Cycle d'Ingénieur") {
          yearLabel = `${filiere.code} ${year}`; // LSI 1, LSI 2, LSI 3
        } else if (filiere.level === 'Master') {
          yearLabel = `${filiere.code} M${year}`; // AI M1, AI M2
        } else if (filiere.level === 'Licence') {
          yearLabel = `${filiere.code}`; // IDAI, AD (Since it's 1 year usually L3)
        }

        docs.push({
          id: idCounter++,
          title: `Liste des Étudiants — ${yearLabel}`,
  ...
  const listDocuments = generateListDocuments();

  const filteredDocs = listDocuments.filter(doc => {
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#f4f7f9', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1a3a6b', color: '#fff', padding: '60px 20px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto', color: '#aec0db', lineHeight: 1.6 }}>
          Consultez et téléchargez les listes officielles des étudiants inscrits pour chaque filière et niveau d'études.
        </p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
          {filteredDocs.map(doc => (
            <div key={doc.id} style={{ background: '#fff', borderRadius: '12px', padding: '24px', border: '1px solid #e2e8f0', transition: 'transform 0.2s', cursor: 'default' }} onMouseOver={e => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseOut={e => e.currentTarget.style.transform = 'translateY(0)'}>
              <div style={{ width: 48, height: 48, borderRadius: '12px', background: '#e0e7ff', color: '#4338ca', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
