...
const TeacherAbsences = () => {
  ...
  const mySessions = useMemo(() =>
    db.sessions.filter(s => s.teacherId === teacherId && s.statut !== 'ANNULEE'),
    [db.sessions, teacherId]);

  const selectedSession = useMemo(() =>
    db.sessions.find(s => s.id === parseInt(selectedSessionId)),
    [db.sessions, selectedSessionId]);
  ...
  useEffect(() => {
    if (selectedSession && selectedDate) {
      ...
      setAbsentStudentIds(existing.map(a => a.studentId));
  ...
  const allMyAbsences = useMemo(() =>
    db.absences
      .filter(a => mySessionIds.has(a.sessionId))
    ...
    if (histSearch) {
      ...
      list = list.filter(a => {
        ...
        return (
          ...
          moduleName(session?.moduleId).toLowerCase().includes(q)
        );
      });
    }
    return list;
  }, [allMyAbsences, histFilter, histSearch, db, moduleName]);
  ...
  const handleSaveAbsences = () => {
    ...
    existing.forEach(a => {
      if (!absentStudentIds.includes(a.studentId)) remove('absences', a.id);
    });

    // Add new absences
    absentStudentIds.forEach(studentId => {
      const exists = existing.find(a => a.studentId === studentId);
      if (!exists) {
        save('absences', {
  ...
  return (
    <div className="animate-up">
      ...
      {justifModal && (
        <JustifModal
          absence={justifModal}
          student={db.students.find(s => s.id === justifModal.studentId)}
          session={db.sessions.find(s => s.id === justifModal.sessionId)}
                      ...
                      {mySessions.map(s => {
                        ...
                        return (
                          <option key={s.id} value={s.id}>
                            {DAY_NAMES[s.day]} · {s.startSlot}–{s.endSlot} · {moduleName(s.moduleId)} ({groupName(s.groupId)}) [{s.type}]
                          </option>
                        );
                      })}
                    </select>
                ...
                {selectedSession && (
                  <div className="abs-session-info-bar">
                    ...
                    <span className="abs-sib-item">
                      ...
                      {moduleName(selectedSession.moduleId)}
                    </span>
                    <span className="abs-sib-item">
                      ...
                      {groupName(selectedSession.groupId)}
                    </span>
                    <span className="abs-sib-item">
                      ...
                      {roomName(selectedSession.roomId)}
                    </span>
                    <span className="abs-sib-item">
                      ...
                      {selectedSession.startSlot} — {selectedSession.endSlot}
                    </span>
                    {saved && (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="abs-student-info">
                            <div className="abs-student-name">{student.name}</div>
                          ...
                          {absentStudentIds.map(sid => {
                            ...
                            return (
                              <div key={sid} style={{ display: 'flex', alignItems: 'center', gap: '.5rem', padding: '.35rem .5rem', background: '#fef2f2', borderRadius: 6 }}>
                                ...
                                <span style={{ fontSize: '.78rem', fontWeight: 600, color: '#991b1b', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</span>
                                <button
                      ...
                      <tr key={a.id}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem' }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 600, fontSize: '.87rem', color: 'var(--text)' }}>{student?.name || '—'}</div>
                              <div style={{ fontSize: '.72rem', color: 'var(--text-3)' }}>{student?.CNE}</div>
                            </div>
                          </div>
                        </td>
                        <td>
                          <div style={{ fontWeight: 600, fontSize: '.85rem', color: 'var(--blue-dark)' }}>{moduleName(session?.moduleId)}</div>
                          <div style={{ fontSize: '.72rem', color: 'var(--text-3)' }}>{session ? `${DAY_NAMES[session.day]} ${session.startSlot}` : '—'}</div>
                        </td>
...
/* ── Justif Modal ────────────────────────────────────────── */
const JustifModal = ({ absence, student, session, moduleName, onAccept, onReject, onClose }) => {
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-bdy">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'var(--surface-2)', borderRadius: 10, marginBottom: '1.25rem' }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', background: `${avatarColor(student?.name || '')}20`, color: avatarColor(student?.name || ''), display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1rem', flexShrink: 0 }}>
              {initials(student?.name || '?')}
            </div>
            <div>
              <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--blue-dark)' }}>{student?.name || '—'}</div>
              <div style={{ fontSize: '.8rem', color: 'var(--text-3)' }}>{student?.CNE} · {student?.email}</div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginBottom: '1.25rem' }}>
            {[
              { label: 'Module', value: moduleName(session?.moduleId) },
              { label: 'Type de séance', value: session?.type || '—' },
              { label: 'Date', value: absence.date ? new Date(absence.date).toLocaleDateString('fr-FR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }) : '—' },
              { label: 'Horaire', value: session ? `${session.startSlot} — ${session.endSlot}` : '—' },
            ].map((item, i) => (
